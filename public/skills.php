...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (!verify_csrf_token($_POST["csrf_token"] ?? null)) {
        ...
        $action = $_POST["action"] ?? "";
        if ($action === "create_skill" && $user["role"] === "admin") {
            $controller->createSkill($_POST, (int) $user["id"]);
        } elseif ($action === "save_employee_skill") {
...
$stmt = $mysqli->prepare("SELECT skill_id, proficiency_level FROM employee_skills WHERE user_id = ?");
...
$mySkills = [];
foreach ($rows as $r) {
    $mySkills[(int) $r["skill_id"]] = $r["proficiency_level"];
}
...
<head>
    ...
    <title>Skills - SME Platform</title>
...
<div class="sidebar-wrapper">
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="<?= e(url('skills.php')) ?>" class="sidebar-link active">
                <span class="sidebar-link-icon">🎯</span>
                <span>Skills</span>
            </a>
    ...
    <div class="main-content">
        ...
        <div class="page-header">
            <div class="container">
                ...
                <p>Manage your skills and proficiencies</p>
            </div>
        </div>
...
<?php if ($user["role"] === "admin"): ?><form method="post" class="row g-2 mb-4"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="create_skill"><div class="col-md-8"><input class="form-control" name="skill_name" placeholder="New skill"></div><div class="col-md-4"><button class="btn btn-primary w-100">Create Skill</button></div></form><?php endif; ?>
<div class="card"><div class="card-body"><h2 class="h6">My Skills</h2>
<?php foreach ($skills as $skill): ?><form method="post" class="row g-2 mb-2"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="action" value="save_employee_skill"><input type="hidden" name="skill_id" value="<?= (int) $skill["id"] ?>"><div class="col-md-6"><label class="form-label"><?= e($skill["skill_name"]) ?></label></div><div class="col-md-4"><select class="form-select" name="proficiency_level"><option value="beginner" <?= (($mySkills[(int)$skill["id"]] ?? "") === "beginner") ? "selected" : "" ?>>Beginner</option><option value="intermediate" <?= (($mySkills[(int)$skill["id"]] ?? "") === "intermediate") ? "selected" : "" ?>>Intermediate</option><option value="expert" <?= (($mySkills[(int)$skill["id"]] ?? "") === "expert") ? "selected" : "" ?>>Expert</option></select></div><div class="col-md-2"><button class="btn btn-outline-primary w-100">Save</button></div></form><?php endforeach; ?>
